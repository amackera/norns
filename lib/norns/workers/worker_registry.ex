defmodule Norns.Workers.WorkerRegistry do
  @moduledoc """
  Tracks connected workers and their tools. Dispatches tool tasks to workers
  and routes results back to waiting agent processes.
  """

  use GenServer

  require Logger

  alias Norns.Tools.Tool
  alias Norns.Workers.TaskQueue

  def start_link(opts \\ []) do
    GenServer.start_link(__MODULE__, opts, name: __MODULE__)
  end

  # -- Public API --

  @doc "Register a worker with its tool definitions."
  def register_worker(tenant_id, worker_id, channel_pid, tools) do
    GenServer.call(__MODULE__, {:register, tenant_id, worker_id, channel_pid, tools})
  end

  @doc "Remove a worker."
  def unregister_worker(tenant_id, worker_id) do
    GenServer.cast(__MODULE__, {:unregister, tenant_id, worker_id})
  end

  @doc "Get all tools from connected workers for a tenant, as %Tool{} structs."
  def available_tools(tenant_id) do
    GenServer.call(__MODULE__, {:available_tools, tenant_id})
  end

  @doc "Dispatch a tool task to a connected worker. Returns {:ok, task_id} or {:error, :no_worker}."
  def dispatch_task(tenant_id, tool_name, input, opts \\ []) do
    GenServer.call(__MODULE__, {:dispatch, tenant_id, tool_name, input, opts})
  end

  @doc "Block until a result arrives for a task. Called from the agent process."
  def await_result(task_id, timeout \\ 300_000) do
    receive do
      {:tool_result, ^task_id, result} -> result
    after
      timeout -> {:error, :timeout}
    end
  end

  @doc "Deliver a result from a worker channel."
  def deliver_result(task_id, payload) do
    GenServer.cast(__MODULE__, {:deliver_result, task_id, payload})
  end

  # -- Callbacks --

  @impl true
  def init(_opts) do
    state = %{
      workers: %{},        # {tenant_id, worker_id} => %{channel_pid, tools, monitor_ref}
      pending: %{}          # task_id => %{from_pid, tenant_id, tool_name}
    }

    {:ok, state}
  end

  @impl true
  def handle_call({:register, tenant_id, worker_id, channel_pid, tools}, _from, state) do
    ref = Process.monitor(channel_pid)
    key = {tenant_id, worker_id}

    worker = %{
      channel_pid: channel_pid,
      tools: tools,
      monitor_ref: ref,
      tenant_id: tenant_id
    }

    state = put_in(state.workers[key], worker)

    # Flush any queued tasks that match this worker's tools
    tool_names = Enum.map(tools, &tool_name/1)

    Enum.each(tool_names, fn name ->
      queued = TaskQueue.flush(tenant_id, name)

      Enum.each(queued, fn task ->
        push_task_to_worker(channel_pid, task)
        state = put_in(state.pending[task.task_id], %{from_pid: task.from_pid, tenant_id: tenant_id, tool_name: name})
        state
      end)
    end)

    {:reply, :ok, state}
  end

  def handle_call({:available_tools, tenant_id}, _from, state) do
    tools =
      state.workers
      |> Enum.filter(fn {{tid, _}, _} -> tid == tenant_id end)
      |> Enum.flat_map(fn {_, worker} -> worker.tools end)
      |> Enum.map(fn tool_def ->
        %Tool{
          name: tool_name(tool_def),
          description: tool_def["description"] || "",
          input_schema: tool_def["input_schema"] || %{},
          handler: fn _ -> {:error, "remote tool — use dispatch"} end,
          source: {:remote, tenant_id}
        }
      end)

    {:reply, tools, state}
  end

  def handle_call({:dispatch, tenant_id, tool_name, input, opts}, _from, state) do
    agent_id = Keyword.get(opts, :agent_id)
    run_id = Keyword.get(opts, :run_id)
    from_pid = Keyword.get(opts, :from_pid, self())

    # Find a worker with the requested tool
    worker =
      state.workers
      |> Enum.find(fn {{tid, _}, w} ->
        tid == tenant_id && Enum.any?(w.tools, &(tool_name(&1) == tool_name))
      end)

    case worker do
      {_key, w} ->
        task_id = generate_task_id()

        push_task_to_worker(w.channel_pid, %{
          task_id: task_id,
          tool_name: tool_name,
          input: input,
          agent_id: agent_id,
          run_id: run_id
        })

        pending = %{from_pid: from_pid, tenant_id: tenant_id, tool_name: tool_name}
        state = put_in(state.pending[task_id], pending)

        {:reply, {:ok, task_id}, state}

      nil ->
        # No worker available — queue the task
        task = %{
          task_id: generate_task_id(),
          tool_name: tool_name,
          input: input,
          from_pid: from_pid,
          agent_id: agent_id,
          run_id: run_id
        }

        TaskQueue.enqueue(tenant_id, task)
        {:reply, {:ok, task.task_id}, state}
    end
  end

  @impl true
  def handle_cast({:unregister, tenant_id, worker_id}, state) do
    key = {tenant_id, worker_id}

    case Map.pop(state.workers, key) do
      {%{monitor_ref: ref}, workers} ->
        Process.demonitor(ref, [:flush])
        {:noreply, %{state | workers: workers}}

      {nil, _} ->
        {:noreply, state}
    end
  end

  def handle_cast({:deliver_result, task_id, payload}, state) do
    case Map.pop(state.pending, task_id) do
      {%{from_pid: pid}, pending} ->
        result =
          case payload do
            %{"status" => "ok", "result" => result} -> {:ok, result}
            %{"status" => "error", "error" => error} -> {:error, error}
            _ -> {:error, "invalid result payload"}
          end

        send(pid, {:tool_result, task_id, result})
        {:noreply, %{state | pending: pending}}

      {nil, _} ->
        Logger.warning("Received result for unknown task: #{task_id}")
        {:noreply, state}
    end
  end

  @impl true
  def handle_info({:DOWN, ref, :process, _pid, _reason}, state) do
    # Worker channel went down — remove the worker
    case Enum.find(state.workers, fn {_, w} -> w.monitor_ref == ref end) do
      {{tenant_id, worker_id} = key, _worker} ->
        Logger.info("Worker #{worker_id} disconnected from tenant #{tenant_id}")
        workers = Map.delete(state.workers, key)
        {:noreply, %{state | workers: workers}}

      nil ->
        {:noreply, state}
    end
  end

  # -- Helpers --

  defp tool_name(%{"name" => name}), do: name
  defp tool_name(name) when is_binary(name), do: name

  defp generate_task_id do
    :crypto.strong_rand_bytes(16) |> Base.url_encode64(padding: false)
  end

  defp push_task_to_worker(channel_pid, task) do
    send(channel_pid, {:push_tool_task, task})
  end
end
