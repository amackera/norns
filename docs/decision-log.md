...
- The provisioner (`norns-provision`, separate Go repo) therefore manages **deployments** — name + image + secrets + workload shape — with a gard as an optional per-deployment property, not the unit itself.
